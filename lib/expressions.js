...
exports.buildFilterExpression = function (key, operator, val1, val2) {
  // IN filter expression is unlike all the others where val1 is an array of values
  if (operator === 'IN') {
    return internals.buildInFilterExpression(key, val1);
  }
...
internals.buildInFilterExpression = function (key, values) {
  var path = '#' + key;

  var attributeNames = {};
  attributeNames[path] = key;

  var attributeValues = _.reduce(values, function(result, val, idx) {
    var p = [':', key, '_', idx +1].join('');
