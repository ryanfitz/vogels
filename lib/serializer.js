...
var serialize = internals.serialize = {
  ...
  objectSet : function (value) {
    var s;
    if(_.isArray(value) ) {
      var stringifiedObjects = _.map(value, function(v) {
          var j;
          try {
              j = JSON.stringify(v);
          } catch (e) {
...
var deserializer = internals.deserializer = {
  ...
  objectSet: function(value, options) {
    var arr;
    if(value.SS) {
      arr = _.map(value.SS, function(s) {
          try {
              return JSON.parse(s);
          } catch (e) {
              return s;
      ...
      if (!options || !options.convertStringToSet) return [value.S];
        
      //check if it is stored as a JSON encoded array
      try {
        arr = JSON.parse(value.S);
        if (_.isArray(arr)) {
          return arr;
        }
        return [value.S];
  ...
  numberSet : function (value, options) {
    if(value.NS) {
      ...
      if (!options || !options.convertStringToSet) return [Number(value.S)];
          
      //check if it is stored as a JSON encoded array
      try {
        var arr = JSON.parse(value.S);
        if (_.isArray(arr)) {
          return _.map(arr, function(numString){return Number(numString);});
        }
        return [Number(value.S)];
      } catch (e) {
        return [Number(value.S)];
  ...
  stringSet : function (value, options) {
    if(value.SS) {
      return value.SS;
    } else if (value.NS) {
      return value.NS;
...
internals.deserializeAttribute = function (value, attr) {
  ...
  case 'object':
    return deserializer.object(value);
  case 'objectSet':
    if (attr.options && attr.options.convertStringToSet)
      return deserializer.objectSet(value, {convertStringToSet: true});
    else
      return deserializer.objectSet(value);        
  case 'boolean':
    return deserializer.boolean(value);
  case 'numberSet':
    if (attr.options && attr.options.convertStringToSet)
      return deserializer.numberSet(value, {convertStringToSet: true});
    else
      return deserializer.numberSet(value);
  case 'stringSet':
    if (attr.options && attr.options.convertStringToSet)
      return deserializer.stringSet(value, {convertStringToSet: true});
    else
...
internals.serializeAttribute = function (value, attr, options) {
  ...
  case 'objectSet':
    return serialize.objectSet(value);
  case 'boolean':
