...
internals.addConditionExpression = function (params, expectedConditions) {
  _.each(expectedConditions, function (val, key) {
    ...
    if (_.isObject(val) && _.isBoolean(val.Exists) && val.Exists === true) {
      condition = expressions.buildFilterExpression(key, 'attribute_exists', existingValueKeys, null, null );
    } else if (_.isObject(val) && _.isBoolean(val.Exists) && val.Exists === false) {
      condition = expressions.buildFilterExpression(key, 'attribute_not_exists', existingValueKeys, null, null);
    } else {
      condition = expressions.buildFilterExpression(key, '=', existingValueKeys,  val, null);
    }
    ...
    if (_.isString(params.KeyConditionExpression )) {
...
Table.prototype.createTable = function (options, callback) {
  ...
  self._dynamodb.createTable(params, callback);
};
...
Table.prototype.describeTable = function (callback) {
  ...
  this._dynamodb.describeTable(params, callback);
};
...
Table.prototype.deleteTable = function (callback) {
  ...
  this._dynamodb.deleteTable(params, callback);
};
...
Table.prototype.updateTable = function (throughput, callback) {
  ...
  this._dynamodb.updateTable(params, callback);
};
