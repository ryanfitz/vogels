...
internals.configSchema = Joi.object().keys({
  ...
  schema    : Joi.object(),
  timestamps : Joi.alternatives().try(Joi.boolean().default(false), Joi.func()),
  ...
  Joi.validate(config, internals.configSchema, { context: context }, function (err, data) {
    ...
    if(self.timestamps) {
      ...
      if(self.createdAt !== false) {
        valids[createdAtParamName] = Joi.alternatives().try(Joi.date(), Joi.number());
      }

      if(self.updatedAt !== false) {
        valids[updatedAtParamName] = Joi.alternatives().try(Joi.date(), Joi.number());
      }
