...
Schema.types.uuid = function (description) {
  var desc = _.isString(description) ? description : 'uuid';
  return Joi.string().guid().default(nodeUUID.v4, desc);
};

Schema.types.timeUUID = function (description) {
  var desc = _.isString(description) ? description : 'timeUUID';
  return Joi.string().guid().default(nodeUUID.v1, desc);
};
